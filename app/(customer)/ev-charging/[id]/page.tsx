  ...
  useEffect(() => {
    ...
    const fetchTracking = async () => {
      try {
        ...
        const res = await apiClient.get(`/charging/${order.id}/tracking`);
        if (!mounted) return;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Live Tracking (MVP) */}
            {order.driver && (order.status === 'ASSIGNED' || order.status === 'IN_PROGRESS') && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold text-gray-900">Live Tracking</h2>
                  <span className="text-xs text-gray-500">updates ~10s</span>
                </div>
